...
import { FormField, Select, Input, Textarea, Tooltip, ColorSelect, PaddingSelect, CollapsibleSection } from '@/components/ui'
...
const ICON_COLOR_KEYS = ['pageColor', 'fontColor', 'backgroundColor', 'borderColor', 'primaryColor', 'secondaryColor', 'alternativeColor', 'designColor', 'brandColor'] as const

function IconColorSelect({ id, value, onChange }: { id: string; value: string; onChange: (v: string) => void }) {
  const { t } = useTranslation()
    ...
    <div className="flex items-center gap-2">
      ...
      >
        {ICON_COLOR_KEYS.map((key) => (
          <option key={key} value={key}>{t(`editor.properties.${key}`)}</option>
        ))}
      </select>
    </div>
  )
...
export function SocialProperties({ block, onChange }: Props) {
  ...
  const deleteIcon = (i: number) => {
    ...
    for (let j = i; j < iconCount - 1; j++) {
      newProps[`icon${j + 1}Type`]  = p[`icon${j + 2}Type`]  ?? DEFAULT_ICON_DEFAULTS[j + 2] ?? 'facebook'
      ...
      >
        {Array.from({ length: iconCount }).map((_, i) => {
          ...
          return (
            <CollapsibleSection
              ...
              onDelete={iconCount > 1 ? () => deleteIcon(i) : undefined}
              deleteTooltip={iconCount > 1 ? t('editor.tree.deleteBlock') : undefined}
              onReset={() => onChange({ [`icon${n}Type`]: DEFAULT_ICON_DEFAULTS[n] ?? 'facebook', [`icon${n}Href`]: '', [`icon${n}Color`]: 'pageColor', [`icon${n}Shape`]: 'rounded', [`icon${n}Bg`]: 'fontColor' })}
            ...
            >
              <FormField label={t('editor.properties.graphic')} htmlFor={`social-icon${n}-type`}>
                <IconSelect
                  ...
                  onChange={(v) => onChange({ [`icon${n}Type`]: v })}
                />
              </FormField>

              <FormField label={t('editor.properties.graphicColor')} htmlFor={`social-icon${n}-color`}>
                <IconColorSelect
                  id={`social-icon${n}-color`}
                  value={p[`icon${n}Color`] ?? 'pageColor'}
                  onChange={(v) => onChange({ [`icon${n}Color`]: v })}
                />
              </FormField>

              <FormField label={t('editor.properties.iconBackground')} htmlFor={`social-icon${n}-shape`}>
                <Select
                  id={`social-icon${n}-shape`}
                  value={p[`icon${n}Shape`] ?? 'rounded'}
                  onChange={(e) => onChange({ [`icon${n}Shape`]: e.target.value })}
                >
              ...
              <FormField label={t('editor.properties.backgroundColorFull')} htmlFor={`social-icon${n}-bg`}>
                <ColorSelect
                  id={`social-icon${n}-bg`}
                  value={String(p[`icon${n}Bg`] ?? 'fontColor')}
                  onChange={(v) => onChange({ [`icon${n}Bg`]: v || undefined })}
                />
              </FormField>
              ...
              <FormField label={t('editor.properties.url')} htmlFor={`social-icon${n}-href`}>
                <Input
                  ...
                  value={p[`icon${n}Href`] ?? ''}
                  placeholder="https://..."
                  onChange={(e) => onChange({ [`icon${n}Href`]: e.target.value })}
                />
              </FormField>
            </CollapsibleSection>
          )
        ...
        {iconCount < 7 && (
          <Button
            ...
            size="sm"
            className="w-full"
            onClick={() => onChange({ iconCount: String(iconCount + 1) })}
          >
