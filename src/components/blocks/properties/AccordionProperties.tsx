...
const DEFAULT_FONT_QUESTION = {
  fontFamily: '',
  fontSize: '',
  questionFontStyle: 'bold',
  color: 'fontColor',
}
const DEFAULT_FONT_ANSWER = {
  answerFontFamily: '',
  answerFontSize: '',
  answerFontStyle: '',
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <CollapsibleSection label={t('editor.properties.sectionFont')} defaultOpen={false}>
        <CollapsibleSection
          label={t('editor.properties.sectionQuestion')}
          onReset={() => {
            onChange(DEFAULT_FONT_QUESTION)
          }}
          tooltip={resetTooltip}
          noToggle
        >
          ...
          <FormField label={t('editor.properties.fontStyle')} htmlFor="acc-q-fontstyle">
            <InheritSelect
              ...
              onChange={(e) => {
                onChange({ questionFontStyle: e.target.value })
              }}
            >
        ...
        <CollapsibleSection
          label={t('editor.properties.sectionAnswer')}
          onReset={() => {
            onChange(DEFAULT_FONT_ANSWER)
          }}
          tooltip={resetTooltip}
          noToggle
        >
          <FormField label={t('editor.properties.fontFamily')} htmlFor="acc-answer-fontfamily">
            <InheritSelect
              ...
              onChange={(e) => {
                onChange({ answerFontFamily: e.target.value })
              }}
            >
              <option value="Arial, sans-serif">Arial</option>
              <option value="'Helvetica Neue', Helvetica, sans-serif">Helvetica</option>
          ...
          <FormField label={t('editor.properties.fontSize')} htmlFor="acc-answer-fontsize">
            <FontSizeSelect
              ...
              basePx={basePx}
              onChange={(e) => {
                onChange({ answerFontSize: e.target.value })
              }}
            />
          </FormField>
          ...
          <FormField label={t('editor.properties.fontStyle')} htmlFor="acc-a-fontstyle">
            <InheritSelect
              ...
              onChange={(e) => {
                onChange({ answerFontStyle: e.target.value })
              }}
            >
              <option value="normal">{t('editor.properties.normal')}</option>
              <option value="bold">{t('editor.properties.bold')}</option>
          ...
          <FormField label={t('editor.properties.fontColorFull')} htmlFor="acc-answer-color">
            <ColorSelect
              ...
              onChange={(v) => {
                onChange({ answerColor: v || undefined })
              }}
            />
          </FormField>
        </CollapsibleSection>
      ...
      {/* Trennlinie */}
      <CollapsibleSection
        label={t('editor.properties.sectionSeparator')}
        onReset={() => {
          onChange(DEFAULT_SEPARATOR)
        }}
        tooltip={resetTooltip}
        defaultOpen={false}
      >
        <FormField label={t('editor.properties.lineWidth')} htmlFor="acc-sep-width">
          <Select
            ...
            onChange={(e) => {
              onChange({ accordionBorderWidth: e.target.value })
            }}
          >
        ...
        <FormField label={t('editor.properties.lineColor')} htmlFor="acc-sep-color">
          <ColorSelect
            ...
            onChange={(v) => {
              onChange({ accordionBorderColor: v })
            }}
          />
        </FormField>
        ...
        <FormField label={t('editor.properties.lineStyle')} htmlFor="acc-sep-style">
          <Select
            ...
            onChange={(e) => {
              onChange({ accordionBorderStyle: e.target.value })
            }}
          >
      ...
      <CollapsibleSection label={t('editor.properties.sectionContent')}>
        {Array.from({ length: count }).map((_, i) => (
          <CollapsibleSection
            ...
            onReset={() => {
              onChange({
                [`faq${i + 1}Question`]: defaultQuestion(i + 1),
                [`faq${i + 1}Answer`]: defaultAnswer(i + 1),
              })
            }}
            tooltip={resetTooltip}
            noToggle
          >
            <div className="flex flex-col gap-1">
              <div className="flex items-center justify-between">
                ...
                >
                  {t('editor.properties.question')}
                </label>
                <FormatButtons
                  id={`acc-q-${i}`}
                  onWrap={(v) => {
                    onChange({ [`faq${i + 1}Question`]: v })
                  }}
                />
              </div>
              <Textarea
                id={`acc-q-${i}`}
                value={p[`faq${i + 1}Question`] || defaultQuestion(i + 1)}
                onChange={(e) => {
                  onChange({ [`faq${i + 1}Question`]: e.target.value })
                }}
                rows={2}
              />
            </div>
            <div className="flex flex-col gap-1">
              <div className="flex items-center justify-between">
                <label
                  ...
                  className="text-xs font-medium text-gray-500 dark:text-gray-400"
                >
                  {t('editor.properties.answer')}
                </label>
                <FormatButtons
                  id={`acc-a-${i}`}
                  onWrap={(v) => {
                    onChange({ [`faq${i + 1}Answer`]: v })
                  }}
                />
              </div>
              <Textarea
                id={`acc-a-${i}`}
                value={p[`faq${i + 1}Answer`] || defaultAnswer(i + 1)}
                onChange={(e) => {
                  onChange({ [`faq${i + 1}Answer`]: e.target.value })
                }}
                rows={2}
              />
            </div>
        ...
        {count < MAX_ITEMS && (
          <Button
            ...
            className="w-full"
            onClick={() => {
              onChange({ faqCount: String(count + 1) })
            }}
          >
            <svg
              width="13"
