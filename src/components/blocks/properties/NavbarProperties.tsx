...
import { FormField, Select, Input, Textarea, ColorSelect, FontSizeSelect, PaddingSelect, InheritSelect, CollapsibleSection } from '@/components/ui'
      ...
      >
        {Array.from({ length: count }).map((_, i) => (
          <CollapsibleSection
            ...
            onDelete={count > 1 ? () => deleteLink(i) : undefined}
            deleteTooltip={count > 1 ? t('editor.tree.deleteBlock') : undefined}
            onReset={() => onChange({ [`link${i + 1}Text`]: '', [`link${i + 1}Href`]: 'https://...' })}
          ...
          >
            <FormField label={t('editor.properties.linkText')} htmlFor={`nav-link${i + 1}-text`}>
              <Input
                ...
                onChange={(e) => onChange({ [`link${i + 1}Text`]: e.target.value })}
              />
            </FormField>
            <FormField label={t('editor.properties.url')} htmlFor={`nav-link${i + 1}-href`}>
              <Input
                ...
                onChange={(e) => onChange({ [`link${i + 1}Href`]: e.target.value })}
              />
            </FormField>
          </CollapsibleSection>
        ))}
        {count < 6 && (
          <Button
            ...
            size="sm"
            className="w-full"
            onClick={() => onChange({ linkCount: String(count + 1) })}
          >
