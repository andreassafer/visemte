...
export function EditorLayout() {
  ...
  const { reorderBlocks, reorderBlockChildren, reorderBlocksInColumn, loadTemplate, clearHistory } =
    useTemplateStore()
  const {
    selectBlock,
    tabPreviewModes,
  ...
  const onDragEnd = (result: DropResult) => {
    ...
    if (srcId.startsWith('tree-child::') && srcId === destId) {
      ...
      if (source.index !== destination.index)
        reorderBlockChildren(blockId, source.index, destination.index)
    }
    ...
    if (srcId.startsWith('tree-col-child::') && srcId === destId) {
      ...
      if (source.index !== destination.index)
        reorderBlocksInColumn(blockId, colIndex, source.index, destination.index)
    }
  }
  ...
  // ── Handle manual save ────────────────────────────────────────────────────────
  const handleSave = () => {
    if (isPresetTab) void storage.savePreset(template)
    else void storage.saveTemplate(template)
  ...
  const handleImportFile = (file: File) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        showToast(t('templates.importSuccess'), 'success')
      } catch {
        showToast(t('errors.invalidTemplate'), 'error')
      }
    }
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd}>
      ...
      <div className="flex h-screen overflow-hidden">
        <BlockPalette />
        <BlockTree />
        <ErrorBoundary label={t('editor.toolbar.settings')}>
          <PropertiesPanel />
        </ErrorBoundary>
        ...
        <div className="flex flex-1 flex-col overflow-hidden">
          <div className="flex flex-shrink-0 items-center justify-center border-b border-gray-200 bg-white px-2 py-[6.5px] dark:border-gray-700 dark:bg-gray-900">
            <div className="flex items-center gap-1">
              ...
              <Tooltip label={t('tooltips.about')} side="bottom">
                ...
                >
                  <svg
                    className="h-[27px] w-[27px] flex-shrink-0 rounded-[5px] overflow-hidden"
              ...
              <Tooltip label={t('editor.toolbar.open')} side="bottom">
                <Button
                  ...
                  aria-label={t('editor.toolbar.open')}
                >
                  <svg
                    width="15"
                    height="15"
              ...
              <Tooltip label={t('tooltips.desktop')} side="bottom">
                <Button
                  ...
                  size="md"
                  onClick={() => {
                    setPreviewMode(activeTabId, 'desktop')
                    selectBlock(null)
                  }}
              ...
              <Tooltip label={t('tooltips.mobile')} side="bottom">
                <Button
                  ...
                  size="md"
                  onClick={() => {
                    setPreviewMode(activeTabId, 'mobile')
                    selectBlock(null)
                  }}
          ...
          <ErrorBoundary label={t('editor.toolbar.preview')}>
