...
export function BlockIcon({ type, size = 14, strokeWidth }: { type: BlockType; size?: number; strokeWidth?: string }) {
  const s = size
  const sw = strokeWidth ?? (s <= 14 ? '1.5' : '1.25')
  switch (type) {
    case 'text':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw} strokeLinecap="round" strokeLinejoin="round"><polyline points="8 7 2 12 8 17" /><polyline points="16 7 22 12 16 17" /><path d="M4 4V1h16v3M9 17h6M12 1v16" /></svg>
    case 'image':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw}><rect x="3" y="3" width="18" height="18" rx="2" /><circle cx="8.5" cy="8.5" r="1.5" /><path d="m21 15-5-5L5 21" /></svg>
    case 'button':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw}><rect x="2" y="7" width="20" height="10" rx="3" /><path d="M8 12h8" /></svg>
    case 'divider':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw}><line x1="3" y1="8" x2="3" y2="16" /><line x1="3" y1="12" x2="21" y2="12" /><line x1="21" y1="8" x2="21" y2="16" /></svg>
    case 'columns':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw}><rect x="3" y="3" width="18" height="18" rx="2" /><line x1="12" y1="3" x2="12" y2="21" /></svg>
    case 'social':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw}><rect x="2" y="7" width="8" height="10" rx="1" /><rect x="14" y="7" width="8" height="10" rx="1" /></svg>
    case 'navbar':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw}><rect x="2" y="4" width="20" height="16" rx="2" /><path d="M6 12h2M11 12h2M16 12h2" /></svg>
    case 'hero':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw}><rect x="3" y="3" width="18" height="18" rx="2" /><path d="M3 9h18M8 15h8M10 12h4" /></svg>
    case 'video':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw}><rect x="2" y="5" width="20" height="14" rx="2" /><polygon points="10 9 16 12 10 15 10 9" fill="currentColor" stroke="none" /></svg>
    case 'countdown':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw}><circle cx="12" cy="12" r="10" /><path d="M12 6v6l4 2" /></svg>
    case 'accordion':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw}><circle cx="12" cy="12" r="10" /><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3" /><line x1="12" y1="17" x2="12.01" y2="17" strokeWidth="2.5" strokeLinecap="round" /></svg>
    case 'quote':
      return <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={sw}><path d="M3 21c3 0 7-1 7-8V5c0-1.25-.756-2.017-2-2H4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2 1 0 1 0 1 1v1c0 1-1 2-2 2s-1 .008-1 1.031V20c0 1 0 1 1 1z" /><path d="M15 21c3 0 7-1 7-8V5c0-1.25-.757-2.017-2-2h-4c-1.25 0-2 .75-2 1.972V11c0 1.25.75 2 2 2h.75c0 2.25.25 4-2.75 4v3c0 1 0 1 1 1z" /></svg>
    default:
      return <span style={{ fontSize: s * 0.6 }} className="font-bold">{(type as string)[0]?.toUpperCase()}</span>
  }
}
...
function useAddBlock() {
  ...
  return (type: BlockType) => {
    addBlock(type)
    const newBlock = useTemplateStore.getState().tabs
      .find((tab) => tab.id === useTemplateStore.getState().activeTabId)
      ?.blocks.at(-1)
...
export function BlockPalette() {
  ...
  const toggle = () => { selectBlock(null); setWide((prev) => !prev) }
  ...
  return (
    ...
    >
      <div className={`flex flex-shrink-0 items-center gap-1.5 bg-gray-100 dark:bg-gray-800 ${wide ? 'px-3 py-2.5' : 'justify-center py-[13px]'}`}>
        ...
        {wide && <span className="text-sm font-medium" style={{ color: "var(--accent)" }}>{t('editor.palette.title')}</span>}
      </div>
      <div className={`flex flex-1 flex-col overflow-y-auto py-1 ${wide ? 'items-stretch px-1' : 'items-center'}`}>
        {PALETTE_ITEMS.map((item) =>
          wide ? (
            ...
            >
              <span className="flex-shrink-0 text-gray-400 dark:text-gray-500"><BlockIcon type={item.type} size={16} /></span>
      ...
      <div className={`flex flex-shrink-0 py-1 ${wide ? 'justify-end px-1' : 'justify-center'}`}>
        <Tooltip label={wide ? t('editor.tree.collapse') : t('editor.tree.expand')} side="right" sideOffset={4}>
          <button
            onClick={toggle}
...
function TreeRow({ block, index, wide, selectedBlockId, hasChildSelected, onSelect, onDuplicate, onToggleDisabled, onDelete, children }: TreeRowProps) {
  const { t } = useTranslation()
      ...
      {(dp, ds) => (
        <div ref={dp.innerRef} {...dp.draggableProps} data-tree-item>
          {!wide ? (
            <Tooltip label={t(`editor.blocks.${block.type}`)} side="right">
              <div
                ...
                onClick={() => onSelect(hasChildSelected ? block.id : selectedBlockId === block.id ? null : block.id)}
              >
                <span className={`flex-shrink-0 ${isDisabled ? 'opacity-40' : 'text-gray-400 dark:text-gray-500'}`}>
          ...
          >
            ...
            <span className={`flex-1 truncate text-sm ${isDisabled ? 'line-through opacity-50' : ''}`}>
            ...
            <Tooltip label={t('common.duplicate')} side="bottom">
              ...
              >
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                  <rect x="9" y="9" width="14" height="14" rx="2" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" />
                </svg>
              </button>
            </Tooltip>
            <Tooltip label={isDisabled ? t('editor.tree.enableBlock') : t('editor.tree.disableBlock')} side="bottom">
              <button
                ...
                onClick={(e) => { e.stopPropagation(); onToggleDisabled(block.id) }}
                aria-label={isDisabled ? t('editor.tree.enableBlock') : t('editor.tree.disableBlock')}
              >
                {isDisabled ? (
                  <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
...
function ColumnChildRow({ block, index, colBlockId, colIndex, wide, isSelected, onSelect, onDelete }: {
  block: EmailBlock; index: number; colBlockId: string; colIndex: number
  wide: boolean; isSelected: boolean
  ...
  return (
    <Draggable draggableId={`tree-col-child::${colBlockId}::${colIndex}::${block.id}`} index={index}>
      {(dp, ds) => (
        <Tooltip label={!wide ? t(`editor.blocks.${block.type}`) : ''} side="right">
          ...
          >
            <span className="flex-shrink-0 text-gray-400 dark:text-gray-500"><BlockIcon type={block.type} /></span>
            {wide && (
              <>
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-0.5 w-36 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-gray-900">
          {COLUMN_CHILD_TYPES.map((type) => (
            ...
            >
              <span className="text-gray-400 dark:text-gray-500"><BlockIcon type={type} size={12} /></span>
...
function TreeChildRows({ block, wide }: { block: EmailBlock; wide: boolean }) {
  const { t } = useTranslation()
  const { selectedBlockId, selectedChildIndex, selectBlock, selectChild, selectColumn } = useEditorStore()
  const { duplicateBlockChild, toggleBlockChildDisabled, removeBlockChild, addBlockToColumn, removeBlockFromColumn } = useTemplateStore()
  ...
  const renderRow = (i: number, icon: React.ReactNode, label: string, dp: DraggableProvided, isDragging: boolean, isDisabled = false, extraActions?: React.ReactNode) => (
    ...
    <div
      ...
      onClick={(e) => handleChildClick(e, i)}
    >
      <span className={`flex-shrink-0 ${isDisabled ? 'opacity-40 text-gray-400 dark:text-gray-500' : 'text-gray-400 dark:text-gray-500'}`}>{icon}</span>
      {wide && <span className={`flex-1 truncate text-sm ${isDisabled ? 'line-through opacity-50' : ''}`}>{label}</span>}
      {wide && (
        <div className="flex items-center gap-1">
          ...
          <Tooltip label={t('common.duplicate')} side="bottom">
            <button
              className="flex-shrink-0 hidden group-hover:flex items-center justify-center rounded p-0.5 text-gray-300 hover:bg-gray-100 hover:text-gray-500 dark:text-gray-600 dark:hover:bg-gray-700 dark:hover:text-gray-300"
              onClick={(e) => { e.stopPropagation(); duplicateBlockChild(block.id, i) }}
              aria-label={t('common.duplicate')}
            >
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
                <rect x="9" y="9" width="14" height="14" rx="2" /><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1" />
          ...
          <Tooltip label={isDisabled ? t('editor.tree.enableBlock') : t('editor.tree.disableBlock')} side="bottom">
            <button
              ...
              onClick={(e) => { e.stopPropagation(); toggleBlockChildDisabled(block.id, i) }}
              aria-label={isDisabled ? t('editor.tree.enableBlock') : t('editor.tree.disableBlock')}
            >
              {isDisabled ? (
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5">
          ...
          <Tooltip label={t('editor.tree.deleteBlock')} side="bottom">
            <button
              className="flex-shrink-0 hidden group-hover:flex items-center justify-center rounded p-0.5 text-gray-300 hover:bg-red-50 hover:text-red-400 dark:text-gray-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
              onClick={(e) => { e.stopPropagation(); removeBlockChild(block.id, i) }}
              aria-label={t('editor.tree.deleteBlock')}
            >
              <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                <polyline points="3 6 5 6 21 6" /><path d="M19 6l-1 14H6L5 6" /><path d="M10 11v6M14 11v6" /><path d="M9 6V4h6v2" />
  ...
  if (block.type === 'columns') {
    ...
    const colPropsArr = (Array.isArray(block.props['columnProps']) ? block.props['columnProps'] : []) as Record<string, string>[]
          ...
          <div ref={dp.innerRef} {...dp.droppableProps}>
            {Array.from({ length: count }, (_, i) => (
              <React.Fragment key={i}>
                    ...
                    wide ? (
                      <InlineAddToColumnButton
                        onAdd={(type) => {
                          ...
                          const newCols = (useTemplateStore.getState().tabs
                            .find((tab) => tab.id === useTemplateStore.getState().activeTabId)
                            ?.blocks.find((b) => b.id === block.id)
                            ?.props['columnBlocks'] as EmailBlock[][] | undefined)?.[i]
                ...
                <Droppable droppableId={`tree-col-child::${block.id}::${i}`} type={`col-child-${block.id}-${i}`}>
                  {(colDp) => (
                    <div ref={colDp.innerRef} {...colDp.droppableProps}>
                      {(columnBlocks[i] ?? []).map((childBlock, ci) => (
                        <ColumnChildRow
                          ...
                          onSelect={() => selectBlock(selectedBlockId === childBlock.id ? null : childBlock.id)}
                          onDelete={() => removeBlockFromColumn(block.id, i, childBlock.id)}
                        />
                      ))}
                      {colDp.placeholder}
                    </div>
            ...
            {tabs.map((tab) => (
              <li key={tab.id}>
                ...
                >
                  ...
                  <span className="flex-1 truncate font-medium">{tab.name || t('editor.untitled')}</span>
                  {isDirty(tab) && (
...
export function BlockTree() {
  ...
  const handleAdd = useAddBlock()
  const handleDuplicate = (id: string) => { duplicateBlock(id, nanoid()) }
    ...
    >
      ...
      <div className={`flex flex-shrink-0 items-center gap-1.5 bg-gray-100 dark:bg-gray-800 ${wide ? 'px-3 py-2.5' : 'justify-center py-[13px]'}`}>
        <Tooltip label={!wide ? t('editor.tree.title') : ''} side="right" sideOffset={12}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="shrink-0" style={{ color: "var(--accent)" }}>
            <rect x="9" y="2" width="6" height="4" rx="1" /><rect x="2" y="17" width="6" height="4" rx="1" /><rect x="16" y="17" width="6" height="4" rx="1" /><path d="M12 6v5M12 11H5v6M12 11h7v6" />
          </svg>
        </Tooltip>
        {wide && <span className="text-sm font-medium" style={{ color: "var(--accent)" }}>{t('editor.tree.title')}</span>}
      </div>
      ...
      >
        ...
        {wide ? (
          <div
            ...
            data-tree-root
            onClick={() => selectBlock(selectedBlockId === '__template__' ? null : '__template__')}
          >
            ...
            {template.blocks.length > 0 && (
              <Tooltip label={t('editor.tree.clearBlocks')} side="bottom">
                <button
                  className="flex-shrink-0 hidden group-hover:flex items-center justify-center rounded p-0.5 text-gray-300 hover:bg-red-50 hover:text-red-400 dark:text-gray-600 dark:hover:bg-red-900/20 dark:hover:text-red-400"
                  onClick={(e) => { e.stopPropagation(); clearBlocks(); selectBlock(null) }}
        ...
        <Droppable droppableId="tree" type="block">
          {(provided, snapshot) => (
            ...
            >
              ...
              {template.blocks.length === 0 && !snapshot.isDraggingOver && (
                wide ? (
                  <button
                    onClick={() => handleAdd(PALETTE_ITEMS[0].type)}
