...
function blockToMjml(
  ...
  switch (block.type) {
    case 'text': {
      ...
      const resolvedInnerPadding =
        resolveRelativePadding(p['innerPadding'], paddingBasePx) ?? settings?.padding ?? '4px'
      if (insideColumn) {
        ...
        const resolvedOuterPadding =
          resolveRelativePadding(p['outerPadding'], paddingBasePx) ?? settings?.padding ?? '4px'
        const outerStyle = [`padding:${resolvedOuterPadding}`, 'box-sizing:border-box'].join(';')
        const innerStyle = [
          resolveColor(p['sectionBg'], settings)
            ? `background-color:${resolveColor(p['sectionBg'], settings)}`
            : '',
      ...
      const standaloneInnerStyle = [
        resolveColor(p['sectionBg'], settings)
          ? `background-color:${resolveColor(p['sectionBg'], settings)}`
          : '',
        p['borderRadius'] && p['borderRadius'] !== '0px'
    ...
    case 'image': {
      ...
      if (insideColumn) {
        ...
        const imgOuterPad =
          resolveRelativePadding(p['outerPadding'], imgPaddingBasePx) ?? settings?.padding ?? '4px'
        ...
        const containerStyle = [
          ...
          .join(';')
        const imgBrStyle =
          p['imgBorderRadius'] && p['imgBorderRadius'] !== '0px'
            ? `border-radius:${p['imgBorderRadius']};`
            : ''
    ...
    case 'button': {
      ...
      const btnBorder =
        p['borderWidth'] && resolveColor(p['borderColor'], settings)
          ? `${p['borderWidth']} ${p['borderStyle'] || 'solid'} ${resolveColor(p['borderColor'], settings)}`
          : 'none'
      if (insideColumn) {
        ...
        const btnOuterPad =
          resolveRelativePadding(p['outerPadding'], btnPaddingBasePx) ?? settings?.padding ?? '4px'
        const btnSecInnerPad =
          resolveRelativePadding(p['secInnerPadding'], btnPaddingBasePx) ??
          settings?.padding ??
          '4px'
        ...
        return `<div class="blk-${btnSanitizedId}" style="${outerStyle}"><div style="${containerStyle}"><a href="${p['href'] ?? '#'}" style="${btnAnchorStyle}">${p['text'] ?? 'Hier klicken'}</a></div></div>`
      }
      const btnPaddingBasePx = parseInt(settings?.padding ?? '4px', 10)
      const resolvedBtnInnerPadding =
        resolveRelativePadding(p['innerPadding'], btnPaddingBasePx) ?? '12px 24px'
      return `<mj-button
        href="${p['href'] ?? '#'}"
        align="${colAlign ?? p['align'] ?? 'center'}"
    ...
    case 'divider': {
      ...
      const lineBorderColor =
        resolveColor(p['lineBorderColor'] ?? p['borderColor'], settings) || '#e5e7eb'
      ...
      if (insideColumn) {
        ...
        const divInnerPad =
          resolveRelativePadding(p['innerPadding'], divPaddingBasePx) ?? settings?.padding ?? '4px'
        const divOuterPad =
          resolveRelativePadding(p['outerPadding'], divPaddingBasePx) ?? settings?.padding ?? '4px'
    ...
    case 'columns': {
      ...
      const enabledIndices = Array.from({ length: count }, (_, i) => i).filter(
        (i) => (colPropsArr[i] ?? {})['disabled'] !== 'true',
      )
      const enabledCount = enabledIndices.length
      const enabledWidthRatiosRaw = widthRatios
        ? enabledIndices.map((i) => widthRatios[i] ?? 1)
        : null
      ...
      // --- MOBILE MODE: each column becomes its own full-width mj-section ---
      if (mobileMode) {
        const sections = enabledIndices.map((i) => {
          ...
          const rawColBg = cp['backgroundColor']
          const bg =
            !isEmpty && rawColBg && rawColBg !== 'backgroundColor'
              ? resolveColor(rawColBg, settings)
              : ''
          ...
          const bgAttr = bg ? ` background-color="${bg}"` : ''
          const brAttr = colBrStr && colBrStr !== '0px' ? ` border-radius="${colBrStr}"` : ''
          const borderAttr = colBw ? ` border="${colBw} ${colBs} ${colBc}"` : ''
          ...
          if (isEmpty) {
            ...
            const placeholder = `<div style="min-height:80px;margin:4px;border:1.5px dashed ${borderClr};border-radius:2px;display:flex;align-items:center;justify-content:center;"><svg width="22" height="30" viewBox="0 0 22 30" fill="none"><rect x="1" y="1" width="20" height="28" rx="2.5" stroke="${iconClr}" stroke-width="1.5"/><text x="11" y="18" text-anchor="middle" font-family="sans-serif" font-size="11" fill="${iconClr}">${colNum}</text></svg></div>`
            return `<mj-section padding="0"${sectionBgAttr}><mj-column padding="${colPad}"${bgAttr}${brAttr}${borderAttr}><mj-text padding="0">${placeholder}</mj-text></mj-column></mj-section>`
          }

          // insideColumn=true returns <div> HTML — wrap in <mj-text> which accepts raw HTML
          const childHtml = children
            .map((cb) => blockToMjml(cb, settings, true, colHAlign))
            .join('\n')
          return `<mj-section padding="0"${sectionBgAttr}><mj-column padding="${colPad}"${bgAttr}${brAttr}${borderAttr}><mj-text padding="0">${childHtml}</mj-text></mj-column></mj-section>`
        })
        return sections.join('\n')
      }

      // --- DESKTOP/EXPORT MODE: native mj-column layout (MJML handles responsive stacking automatically) ---
      const mjColumns = enabledIndices.map((i, arrIdx) => {
        const cp = colPropsArr[i] ?? {}
        const children = (columnBlocks[i] ?? []).filter((cb) =>
          ['text', 'image', 'button', 'divider'].includes(cb.type),
        )
        ...
        const colPad = cp['padding'] || '0px'
        const valign =
          cp['verticalAlign'] === 'middle'
            ? 'middle'
            : cp['verticalAlign'] === 'bottom'
              ? 'bottom'
              : 'top'
        const cellWidth = enabledWidthRatiosRaw
          ? ((enabledWidthRatiosRaw[arrIdx] ?? 0) / enabledTotalRatio) * 100
          : 100 / enabledCount
        const bgAttr = bg ? ` background-color="${bg}"` : ''
        const brAttr = colBrStr && colBrStr !== '0px' ? ` border-radius="${colBrStr}"` : ''
        const borderAttr = colBw ? ` border="${colBw} ${colBs} ${colBc}"` : ''
        ...
        if (isEmpty) {
          ...
          const iconClr = `rgba(${r},${g},${b},0.4)`
          const colNum = cp['col'] ? Number(cp['col']) : i + 1
          const placeholder = `<div style="min-height:120px;margin:4px;border:1.5px dashed ${borderClr};border-radius:2px;display:flex;align-items:center;justify-content:center;"><svg width="22" height="30" viewBox="0 0 22 30" fill="none"><rect x="1" y="1" width="20" height="28" rx="2.5" stroke="${iconClr}" stroke-width="1.5"/><text x="11" y="18" text-anchor="middle" font-family="sans-serif" font-size="11" fill="${iconClr}">${colNum}</text></svg></div>`
          return `<mj-column width="${cellWidth.toFixed(1)}%" padding="${colPad}"${bgAttr}${brAttr}${borderAttr} vertical-align="${valign}"><mj-text padding="0">${placeholder}</mj-text></mj-column>`
        }

        const childHtml = children
          .map((cb) => blockToMjml(cb, settings, true, colHAlign))
          .join('\n')
        return `<mj-column width="${cellWidth.toFixed(1)}%" padding="${colPad}"${bgAttr}${brAttr}${borderAttr} vertical-align="${valign}"><mj-text padding="0">${childHtml}</mj-text></mj-column>`
      })

      return `<mj-section padding="0"${sectionBgAttr} css-class="blk-${sanitizedId}">${mjColumns.join('\n')}</mj-section>`
    }
    ...
    case 'hero': {
      ...
      const heroSrc = p['src'] ?? ''
      const heroBg =
        resolveColor(p['backgroundColor'], settings) || settings?.backgroundColor || '#1e3a5f'
      ...
      const heroBorderRadiusAttr =
        heroBrVal && heroBrVal !== '0px' ? ` border-radius="${heroBrVal}"` : ''

      // Padding (matching other blocks)
      const heroPaddingBasePx = parseInt(settings?.padding ?? '4px', 10)
      const heroOuterPad =
        resolveRelativePadding(p['outerPadding'], heroPaddingBasePx) ??
        getDefaultSectionPadding(settings)
      const heroInnerPad =
        resolveRelativePadding(p['innerPadding'], heroPaddingBasePx) ?? settings?.padding ?? '4px'

      const line1Style = p['line1FontStyle'] || settings?.fontStyle || ''
      const resolvedLine1FontSize =
        resolveRelativeFontSize(p['line1FontSize'], basePx, 40) ?? `${Math.round(basePx * 2)}px`
      const line1Attrs = [
      ...
      const resolvedLine2FontSize =
        resolveRelativeFontSize(p['line2FontSize'], basePx) ?? `${basePx}px`
      const line2Attrs = [
...
${heroLines}
    ...
    case 'social': {
      ...
      const ICON_SVGS_MJML: Record<string, string> = {
        ...
        twitter:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.747l7.73-8.835L2.25 2.25h6.978l4.255 5.623L18.244 2.25zm-1.161 17.52h1.833L7.084 4.126H5.117L17.083 19.77z"/></svg>',
        instagram:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/></svg>',
        linkedin:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433a2.062 2.062 0 0 1-2.063-2.065 2.064 2.064 0 1 1 2.063 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>',
        youtube:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/></svg>',
        tiktok:
        ...
        pinterest:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M12 0C5.373 0 0 5.373 0 12c0 5.084 3.163 9.426 7.627 11.174-.105-.949-.2-2.405.042-3.441.218-.937 1.407-5.965 1.407-5.965s-.359-.719-.359-1.782c0-1.668.967-2.914 2.171-2.914 1.023 0 1.518.769 1.518 1.69 0 1.029-.655 2.568-.994 3.995-.283 1.194.599 2.169 1.777 2.169 2.133 0 3.772-2.249 3.772-5.495 0-2.873-2.064-4.882-5.012-4.882-3.414 0-5.418 2.561-5.418 5.207 0 1.031.397 2.138.893 2.738a.36.36 0 0 1 .083.345l-.333 1.36c-.053.22-.174.267-.402.161-1.499-.698-2.436-2.889-2.436-4.649 0-3.785 2.75-7.262 7.929-7.262 4.163 0 7.398 2.967 7.398 6.931 0 4.136-2.607 7.464-6.227 7.464-1.216 0-2.359-.632-2.75-1.378l-.748 2.853c-.271 1.043-1.002 2.35-1.492 3.146C9.57 23.812 10.763 24 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0z"/></svg>',
        discord:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M20.317 4.37a19.791 19.791 0 0 0-4.885-1.515.074.074 0 0 0-.079.037c-.21.375-.444.864-.608 1.25a18.27 18.27 0 0 0-5.487 0 12.64 12.64 0 0 0-.617-1.25.077.077 0 0 0-.079-.037A19.736 19.736 0 0 0 3.677 4.37a.07.07 0 0 0-.032.027C.533 9.046-.32 13.58.099 18.057a.082.082 0 0 0 .031.057 19.9 19.9 0 0 0 5.993 3.03.078.078 0 0 0 .084-.028 14.09 14.09 0 0 0 1.226-1.994.076.076 0 0 0-.041-.106 13.107 13.107 0 0 1-1.872-.892.077.077 0 0 1-.008-.128 10.2 10.2 0 0 0 .372-.292.074.074 0 0 1 .077-.01c3.928 1.793 8.18 1.793 12.062 0a.074.074 0 0 1 .078.01c.12.098.246.198.373.292a.077.077 0 0 1-.006.127 12.299 12.299 0 0 1-1.873.892.077.077 0 0 0-.041.107c.36.698.772 1.362 1.225 1.993a.076.076 0 0 0 .084.028 19.839 19.839 0 0 0 6.002-3.03.077.077 0 0 0 .032-.054c.5-5.177-.838-9.674-3.549-13.66a.061.061 0 0 0-.031-.03zM8.02 15.33c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.956-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.956 2.418-2.157 2.418zm7.975 0c-1.183 0-2.157-1.085-2.157-2.419 0-1.333.955-2.419 2.157-2.419 1.21 0 2.176 1.096 2.157 2.42 0 1.333-.946 2.418-2.157 2.418z"/></svg>',
        telegram:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>',
        whatsapp:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 0 1-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 0 1-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 0 1 2.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0 0 12.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 0 0 5.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 0 0-3.48-8.413z"/></svg>',
        email:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4-8 5-8-5V6l8 5 8-5v2z"/></svg>',
        phone:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M6.62 10.79c1.44 2.83 3.76 5.14 6.59 6.59l2.2-2.2c.27-.27.67-.36 1.02-.24 1.12.37 2.33.57 3.57.57.55 0 1 .45 1 1V20c0 .55-.45 1-1 1-9.39 0-17-7.61-17-17 0-.55.45-1 1-1h3.5c.55 0 1 .45 1 1 0 1.25.2 2.45.57 3.57.11.35.03.74-.25 1.02l-2.2 2.2z"/></svg>',
        globe:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1 17.93c-3.95-.49-7-3.85-7-7.93 0-.62.08-1.21.21-1.79L9 15v1c0 1.1.9 2 2 2v1.93zm6.9-2.54c-.26-.81-1-1.39-1.9-1.39h-1v-3c0-.55-.45-1-1-1H8v-2h2c.55 0 1-.45 1-1V7h2c1.1 0 2-.9 2-2v-.41c2.93 1.19 5 4.06 5 7.41 0 2.08-.8 3.97-2.1 5.39z"/></svg>',
        github:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M12 .297c-6.63 0-12 5.373-12 12 0 5.303 3.438 9.8 8.205 11.385.6.113.82-.258.82-.577 0-.285-.01-1.04-.015-2.04-3.338.724-4.042-1.61-4.042-1.61C4.422 18.07 3.633 17.7 3.633 17.7c-1.087-.744.084-.729.084-.729 1.205.084 1.838 1.236 1.838 1.236 1.07 1.835 2.809 1.305 3.495.998.108-.776.417-1.305.76-1.605-2.665-.3-5.466-1.332-5.466-5.93 0-1.31.465-2.38 1.235-3.22-.135-.303-.54-1.523.105-3.176 0 0 1.005-.322 3.3 1.23.96-.267 1.98-.399 3-.405 1.02.006 2.04.138 3 .405 2.28-1.552 3.285-1.23 3.285-1.23.645 1.653.24 2.873.12 3.176.765.84 1.23 1.91 1.23 3.22 0 4.61-2.805 5.625-5.475 5.92.42.36.81 1.096.81 2.22 0 1.606-.015 2.896-.015 3.286 0 .315.21.69.825.57C20.565 22.092 24 17.592 24 12.297c0-6.627-5.373-12-12-12"/></svg>',
        ...
        trophy:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M19 5h-2V3H7v2H5c-1.1 0-2 .9-2 2v1c0 2.55 1.92 4.63 4.39 4.94.63 1.5 1.98 2.63 3.61 2.96V19H7v2h10v-2h-4v-3.1c1.63-.33 2.98-1.46 3.61-2.96C19.08 12.63 21 10.55 21 8V7c0-1.1-.9-2-2-2zM5 8V7h2v3.82C5.84 10.4 5 9.3 5 8zm7 8c-1.65 0-3-1.35-3-3V5h6v8c0 1.65-1.35 3-3 3zm5-5.18V7h2v1c0 1.3-.84 2.4-2 2.82z"/></svg>',
        crown:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M5 16L3 5l5.5 5L12 4l3.5 6L21 5l-2 11H5zm0 2h14v2H5v-2z"/></svg>',
        diamond:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M12 2L2 8l10 14L22 8z"/></svg>',
        heart:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/></svg>',
        ...
        sparkle:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M9.937 15.5A2 2 0 0 0 8.5 14.063l-6.135-1.582a.5.5 0 0 1 0-.962L8.5 9.936A2 2 0 0 0 9.937 8.5l1.582-6.135a.5.5 0 0 1 .963 0L14.063 8.5A2 2 0 0 0 15.5 9.937l6.135 1.581a.5.5 0 0 1 0 .964L15.5 14.063a2 2 0 0 0-1.437 1.437l-1.582 6.135a.5.5 0 0 1-.963 0z"/></svg>',
        check:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48" fill="FILL"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg>',
        number1:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48"><text x="12" y="18.5" text-anchor="middle" fill="FILL" font-size="19" font-weight="bold" font-family="Arial,sans-serif">1</text></svg>',
        number2:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48"><text x="12" y="18.5" text-anchor="middle" fill="FILL" font-size="19" font-weight="bold" font-family="Arial,sans-serif">2</text></svg>',
        number3:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48"><text x="12" y="18.5" text-anchor="middle" fill="FILL" font-size="19" font-weight="bold" font-family="Arial,sans-serif">3</text></svg>',
        question:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48"><text x="12" y="18.5" text-anchor="middle" fill="FILL" font-size="19" font-weight="bold" font-family="Arial,sans-serif">?</text></svg>',
        exclamation:
          '<svg xmlns="http://www.w3.org/2000/svg" viewBox="-12 -12 48 48"><text x="12" y="18.5" text-anchor="middle" fill="FILL" font-size="19" font-weight="bold" font-family="Arial,sans-serif">!</text></svg>',
      }
      ...
      if (p['networks'] !== undefined) {
        ...
        elements = Array.from({ length: iconCount }, (_, i) => {
          ...
          const bgColor = noBackground
            ? 'transparent'
            : resolveColor(p[`icon${n}Bg`], settings) ||
              settings?.fontColor ||
              (ICON_COLORS_MJML[type] ?? '#6b7280')
          const iconColor =
            resolveColor(p[`icon${n}Color`], settings) ||
            (noBackground ? (ICON_COLORS_MJML[type] ?? '#6b7280') : '#ffffff')
          ...
          // Use 'custom' as name to prevent MJML from inserting default URLs for known social networks
          return `<mj-social-element name="custom"${src ? ` src="${src}"` : ''} background-color="${bgColor}"${hrefAttr} icon-size="${iconSizePx}" padding="4px 8px" border-radius="${borderRadius}"></mj-social-element>`
        })
          .filter(Boolean)
          .join('\n')
      }
      const socialBg = resolveColor(p['sectionBg'], settings)
        ? ` background-color="${resolveColor(p['sectionBg'], settings)}"`
        : ''
    ...
    case 'navbar': {
      ...
      const textAttrs = [
        ...
        .filter(Boolean)
        .join(' ')
      const navBg = resolveColor(p['sectionBg'], settings)
        ? ` background-color="${resolveColor(p['sectionBg'], settings)}"`
        : ''
    ...
    case 'video': {
      ...
      const paddingBasePx = parseInt(settings?.padding ?? '4px', 10)
      const outerPadding =
        resolveRelativePadding(p['outerPadding'], paddingBasePx) ??
        getDefaultSectionPadding(settings)
      const vidBorderRadius =
        p['borderRadius'] && p['borderRadius'] !== '0px'
          ? ` border-radius="${p['borderRadius']}"`
          : ''
      const phColor = settings?.fontColor ?? '#000000'
      ...
      const imgTag = `<mj-image
        ...
        padding="0"
      />`
      const vidColBg = resolveColor(p['sectionBg'], settings)
        ? ` background-color="${resolveColor(p['sectionBg'], settings)}"`
        : ''
    ...
    case 'countdown': {
      const cdBg = resolveColor(p['sectionBg'], settings)
        ? ` background-color="${resolveColor(p['sectionBg'], settings)}"`
        : ''
    ...
    case 'accordion': {
      const accColBg = resolveColor(p['sectionBg'], settings)
        ? ` background-color="${resolveColor(p['sectionBg'], settings)}"`
        : ''
      ...
      if (p['items'] !== undefined) {
        try {
          accItems = JSON.parse(String(p['items'])) as typeof accItems
        } catch {
          accItems = []
        }
      } else {
        ...
        accItems = Array.from({ length: accCount }, (_, i) => ({
          question:
            p[`faq${i + 1}Question`] || `${i18n.t('editor.blocks.accordionQuestion')} ${i + 1}`,
      ...
      const bdrColor =
        resolveColor(p['accordionBorderColor'] ?? p['borderColor'], settings) || '#e5e7eb'
      ...
      const accPad =
        resolveRelativePadding(p['padding'], accPaddingBasePx) ?? settings?.padding ?? '4px'
...
${isLast ? '' : divider}`
    ...
    case 'quote': {
      ...
      const colBgAttr = resolveColor(p['sectionBg'], settings)
        ? ` background-color="${resolveColor(p['sectionBg'], settings)}"`
        : ''
      ...
      if (quoteStyle === 'quotes') {
        ...
        // none
        const authorRow = authorText
          ? `<tr><td style="padding:0;${authorStyle}">${authorText}</td></tr>`
          : ''
...
export function templateToMjml(template: EmailTemplate, mobileMode = false): string {
  _previewCss.clear()
  ...
  const sectionsHtml = blocks
    .map((block) => {
      ...
      if (
        ...
        return blockToMjml(block, settings, false, undefined, mobileMode)
      }
      ...
      if (block.type === 'text' && bp['width'] && bp['width'] !== '100%') {
        ...
        const txtBgAttr = resolveColor(bp['sectionBg'], settings)
          ? ` background-color="${resolveColor(bp['sectionBg'], settings)}"`
          : ''
        const innerStyle = [
          `width:${bp['width']}`,
        ...
        const textAttrs = [
          `align="${effectiveAlign}"`,
          resolveColor(bp['color'], settings)
            ? `color="${resolveColor(bp['color'], settings)}"`
            : '',
      ...
      const paddingBasePx = parseInt(settings?.padding ?? '4px', 10)
      const sectionBgAttr = resolveColor(bp['sectionBg'], settings)
        ? ` background-color="${resolveColor(bp['sectionBg'], settings)}"`
        : ''
      ...
      const bcKey =
        isButton || isDivider
          ? (bp['secBorderColor'] ?? 'borderColor')
          : (bp['borderColor'] ?? 'borderColor')
      // Text blocks: column has no padding (inner div handles it), matching in-column rendering
      const innerPaddingKey = isButton
        ? (resolveRelativePadding(bp['secInnerPadding'], paddingBasePx) ??
          settings?.padding ??
          '4px')
        : isText
          ? '0px'
          : (resolveRelativePadding(bp['innerPadding'], paddingBasePx) ??
            settings?.padding ??
            '4px')
  ...
  for (const block of blocks) {
    ...
    if (bp['css']?.trim()) {
      ...
      const finalCss = processed.endsWith('}')
        ? processed
        : processed.endsWith(';')
          ? processed
          : processed + ' !important'
  ...
  const resolvedOuterPadding =
    resolveRelativePadding(settings.bodyPadding, globalBasePx) ?? `${globalBasePx}px`
  // Innenabstand: sits INSIDE the border → mj-wrapper's own padding attribute
  const resolvedInnerPadding =
    resolveRelativePadding(settings.bodyInnerPadding, globalBasePx) ??
    getDefaultSectionPadding(settings)
  const wrapperBgColor =
    resolveColor(settings.bodyBackgroundColor, settings) ||
    settings.pageColor ||
    settings.backgroundColor ||
    '#ffffff'
  const resolvedBodyBorderWidth = settings.bodyBorderWidth || '0px'
  const resolvedBodyBorderStyle = settings.bodyBorderStyle || settings.defaultBorderStyle || 'solid'
  const resolvedBodyBorderRadius =
    settings.bodyBorderRadius || settings.defaultBorderRadius || '0px'
  const innerRadiusPx =
    resolvedBodyBorderRadius !== '0px' ? parseInt(resolvedBodyBorderRadius, 10) : 0
  ...
  const outerCssParts = [
    ...
    outerDivRadiusPx > 0
      ? `border-radius:${outerDivRadiusPx}px!important;overflow:hidden!important;`
      : '',
  ]
    .filter(Boolean)
    .join('')
  ...
  if (innerRadiusPx > 0) {
    _previewCss.set(
      ...
      `.body-outer-margin>table>tbody>tr>td{clip-path:inset(0 round ${resolvedBodyBorderRadius})!important;}`,
    )
  } else {
  ...
  const wrapperAttrs = [
    ...
    resolvedBodyBorderWidth !== '0px'
      ? `border="${resolvedBodyBorderWidth} ${resolvedBodyBorderStyle} ${resolveColor(settings.bodyBorderColor, settings) || '#e5e7eb'}"`
      : '',
    innerRadiusPx > 0 ? `border-radius="${resolvedBodyBorderRadius}"` : '',
  ]
    .filter(Boolean)
    .join(' ')
  ...
  <mj-head>
    ...
    <mj-style>.navbar-link:hover { text-decoration: underline !important; }${mobileColStyles}${[..._previewCss.values()].join('')}${(() => {
      const vars = [
        settings.primaryColor ? `--primary:${settings.primaryColor}` : '',
        settings.secondaryColor ? `--secondary:${settings.secondaryColor}` : '',
        settings.alternativeColor ? `--alternative:${settings.alternativeColor}` : '',
        settings.designColor ? `--design:${settings.designColor}` : '',
        settings.brandColor ? `--brand:${settings.brandColor}` : '',
        settings.borderColor ? `--border:${settings.borderColor}` : '',
      ].filter(Boolean)
      return vars.length ? `:root{${vars.join(';')}}` : ''
    })()}${settings.customCss ? `\n${settings.customCss}` : ''}</mj-style>
  </mj-head>
  <mj-body background-color="${settings.pageColor ?? settings.backgroundColor}" width="${mobileMode ? 375 : settings.contentWidth}px">
    ${wrapperHtml}
  </mj-body>
</mjml>`
}
